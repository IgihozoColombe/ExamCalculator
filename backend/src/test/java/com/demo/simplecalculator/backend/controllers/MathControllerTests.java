...
public class MathControllerTests {
    ...
    final static String add = DoMathRequest.SignSymbols.get(DoMathRequest.Sign.PLUS);
    final static String sub = DoMathRequest.SignSymbols.get(DoMathRequest.Sign.MINUS);
    final static String mul = DoMathRequest.SignSymbols.get(DoMathRequest.Sign.MULTIPLICATION);
    final static String div = DoMathRequest.SignSymbols.get(DoMathRequest.Sign.DIVISION);
    ...
    @BeforeEach
    void setup(){
        when(calculatorHistoryService.save(any(DoMathRequest.class))).thenAnswer(i -> {
            DoMathRequest eq = (DoMathRequest) i.getArguments()[0];
            eq.setId(1);
            return eq;
        });
    }
    ...
    @Test
    void getDoMathSolution() throws Exception {
    ...
    void getHistory() throws JsonProcessingException, Exception{
        ...
        List<DoMathRequest> equationList = new ArrayList();
        for ( int i = 0; i < testPages * pageSize; i++ ) {
            equationList.add(new DoMathRequest(100, 20, 50, DoMathRequest.Sign.PLUS, 70, null));
        }
    ...
    void ensureClientTokenSecurity() throws Exception {
        ...
        when(calculatorHistoryService.getHistoryByPage(any(Integer.class), nullable(String.class))).thenAnswer(i -> {
            ...
            if ( "TOKEN-TEST-12345".equals(clientToken) ) {
                return new ArrayList(){{ add(new DoMathRequest(123,456,DoMathRequest.Sign.PLUS,579)); }};
            } else if ( "TOKEN-TEST-67890".equals(clientToken) ) {
                return new ArrayList(){{ add(new DoMathRequest(678,90,DoMathRequest.Sign.PLUS,768)); }};
            }  
            return new ArrayList(){{ add(new DoMathRequest(0,0,DoMathRequest.Sign.PLUS,0)); }};
        });
